#Script to generate the WER for each model in the M2 files compared to multiple references.
#The model considers the best WER among multiple references for each sentence.
...
for f in F:
	...
	for i in range(L):
		...
		#		tmp += k[1]
		tmp = jiwer.wer(trgTxt[i], moTxt[i])
	...
	allRes[f.split("/")[-1].split(".")[0]] = dfTmp["NumEditedWords"].tolist()
...
print("The sentence average WER for each model is:")
#print(tmp)
for i in tmp:
